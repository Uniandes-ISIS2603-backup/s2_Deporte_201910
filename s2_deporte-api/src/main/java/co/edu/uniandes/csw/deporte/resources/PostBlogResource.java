...
import co.edu.uniandes.csw.deporte.ejb.PostBlogLogic;
 ...
 * @author estudiante
 */
@Path("posts/{postId: \\d+}/blog")
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class PostBlogResource {
    private static final Logger LOGGER = Logger.getLogger(PostBlogResource.class.getName());

    @Inject
    private PostLogic bookLogic; // Variable para acceder a la lógica de la aplicación. Es una inyección de dependencias.

    @Inject
    private PostBlogLogic bookEditorialLogic; // Variable para acceder a la lógica de la aplicación. Es una inyección de dependencias.

    @Inject
    private BlogLogic editorialLogic; // Variable para acceder a la lógica de la aplicación. Es una inyección de dependencias
     ...
     */
    @PUT
    public PostDTO replaceEditorial(@PathParam("postId") Long postId, BlogDTO editorial) {
        ...
        if (editorialLogic.getBlog(editorial.getId()) == null) {
            throw new WebApplicationException("El recurso /blog/" + editorial.getId() + " no existe.", 404);
        }
        PostDTO bookDetailDTO = new PostDTO(bookEditorialLogic.replaceBlog(postId, editorial.getId()));
