...
import co.edu.uniandes.csw.deporte.ejb.BlogPostsLogic;
...
public class BlogPostsResource {
     private static final Logger LOGGER = Logger.getLogger(BlogPostsResource.class.getName());

    @Inject
    private BlogPostsLogic editorialBooksLogic; // Variable para acceder a la lógica de la aplicación. Es una inyección de dependencias.
     ...
     *
     * @param blogId Identificador de la editorial que se esta
    ...
    @POST
    @Path("{postId: \\d+}")
    public PostDTO addPost(@PathParam("blogId") Long blogId, @PathParam("postId") Long postId) {
        LOGGER.log(Level.INFO, "EditorialBooksResource addBook: input: editorialsID: {0} , booksId: {1}", new Object[]{blogId, postId});
        if (bookLogic.getPost(postId) == null) {
            throw new WebApplicationException("El recurso /post/" + postId + " no existe.", 404);
        }
        PostDTO bookDTO = new PostDTO(editorialBooksLogic.addPost(postId, blogId));
     ...
     *
     * @param blogId Identificador de la editorial que se esta buscando.
     ...
     */
    @GET
    public List<PostDTO> getBooks(@PathParam("blogId") Long blogId) {
        LOGGER.log(Level.INFO, "EditorialBooksResource getBooks: input: {0}", blogId);
        List<PostDTO> listaDetailDTOs = booksListEntity2DTO(editorialBooksLogic.getPosts(blogId));
    ...
    @GET
    @Path("{postId: \\d+}")
    public PostDTO getBook(@PathParam("blogId") Long blogId, @PathParam("postId") Long postId) throws BusinessLogicException {
        LOGGER.log(Level.INFO, "EditorialBooksResource getBook: input: editorialsID: {0} , booksId: {1}", new Object[]{blogId, postId});
        if (bookLogic.getPost(postId) == null) {
            throw new WebApplicationException("El recurso /editorials/" + blogId + "/books/" + postId + " no existe.", 404);
        }
        PostDTO bookDetailDTO = new PostDTO(editorialBooksLogic.getPost(blogId, postId));
     ...
     */
    @PUT
    public List<PostDTO> replacePosts(@PathParam("blogId") Long blogId, List<PostDTO> posts) {
        LOGGER.log(Level.INFO, "EditortalBooksResource replaceBooks: input: editorialsId: {0} , books: {1}", new Object[]{blogId, posts});
        ...
        List<PostDTO> listaDetailDTOs = booksListEntity2DTO(editorialBooksLogic.replacePosts(blogId, booksListDTO2Entity(posts)));
